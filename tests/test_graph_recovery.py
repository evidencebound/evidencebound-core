...
def cp(name, deps=()): return Checkpoint(name, "a", E, {"n": name}, P, tuple(deps))


def test_exact_blast_radius_branch_case():
    graph = DependencyGraph([cp("A"), cp("B", ["A"]), cp("C", ["B"]), cp("D", ["C"]), cp("X", ["A"])])
...
def test_diamond_and_deterministic_order():
    graph = DependencyGraph([cp("A"), cp("B", ["A"]), cp("C", ["A"]), cp("D", ["B", "C"])])
...
def test_cycle_missing_dependency_and_duplicate_rejected():
    with pytest.raises(GraphError): DependencyGraph([cp("A", ["B"]), cp("B", ["A"])])
    with pytest.raises(GraphError): DependencyGraph([cp("A", ["Z"])])
    with pytest.raises(GraphError): DependencyGraph([cp("A"), cp("A")])


def test_recovery_reusable_recompute_and_blocked():
    graph = DependencyGraph([cp("A"), cp("B", ["A"]), cp("C", ["B"]), cp("X", ["A"])])
    plan = plan_recovery(graph, {"B": InvalidationReason.EVIDENCE_CHANGED}, consequential={"C"})
...
def test_consequential_action_blocked_until_reverification_succeeds():
    ...
    plan = plan_recovery(graph, {"A": InvalidationReason.EVIDENCE_CHANGED}, consequential={"B"})
    ...
    assert action_after_recovery("B", plan, reverified={"B": blocked_result}) is ActionDecision.BLOCK
    allowed_result = verify_checkpoint(cp("B", ["A"]))
    assert action_after_recovery("B", plan, reverified={"B": allowed_result}) is ActionDecision.ALLOW
