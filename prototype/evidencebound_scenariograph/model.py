"""Narrow AI-BOOST Challenge 4 ScenarioGraph prototype.

This module is challenge-specific foreground work.  It intentionally depends on the
pre-existing EvidenceBound Core only for deterministic domain-separated receipts.
"""
...
class EvidenceItem:
    """A source-addressed accident/standards evidence item."""
...
@dataclass(frozen=True)
class ScenarioField:
    """A generated or derived scenario claim with explicit evidence bindings.

    ``uncertainty_milli`` is an integer in [0, 1000] so deterministic receipts never
    depend on floating-point canonicalization.  0 means no recorded uncertainty and
    1000 means maximally uncertain/unknown.
    """
...
@dataclass
class ScenarioGraph:
    """Evidence-linked scenario fields plus deterministic trust/recovery semantics."""
    ...
    def assess_field(self, field_id: str) -> FieldAssessment:
        ...
        if scenario_field.material and scenario_field.value is not None and not scenario_field.evidence_ids:
        ...
        dependency_assessments = [
            self.assess_field(dependency) for dependency in scenario_field.depends_on_fields
        ]
        ...
        if any(item.decision is FieldDecision.REVIEW_REQUIRED for item in dependency_assessments):
            return FieldAssessment(
                field_id=field_id,
                decision=FieldDecision.REVIEW_REQUIRED,
